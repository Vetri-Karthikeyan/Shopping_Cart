...
namespace Shopping_Cart_API.DataAccess
{
    public class DataAccess : IDataAccess
    {
        ...
        public bool InsertUser(User user)
        {
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                query = "INSERT INTO Users (FirstName, LastName, Address, Mobile, Email, Password, CreatedAt, ModifiedAt,Role,Status) values (@fn, @ln, @add, @mb, @em, @pwd, @cat, @mat,'User','Active');";
        ...
        public string IsUserPresent(string email, string password)
        {
            ...
            using (SqlConnection connection = new(dbconnection))
            {
                ...
                while (reader.Read())
                {
                    ...
                    user.CreatedAt = (string)reader["CreatedAt"];
                    user.ModifiedAt = (string)reader["ModifiedAt"];
                    user.UserType = (string)reader["Role"];
                    user.Status = (string)reader["Status"];
                }

                string key = "MNU66iBl3T5rh6H52i69";
                string duration = "60";
                var symmetrickey = new SymmetricSecurityKey(Encoding.UTF32.GetBytes(key));
